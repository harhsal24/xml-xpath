...
function registerCommands(context) {
  const commands = {
    ...
    "xmlXpath.clearParent": () =>
      updateConfig("parentTag", null, "Parent tag cleared."),
...
async function setPredicateTemplate() {
  const cfg = vscode.workspace.getConfiguration(CONFIG_SECTION);
  const currentValue = cfg.get("predicateTemplate", "[{at}{attr1}='{attr1V}']");

  const examples = [
    { label: "Default: [@attr='value']", value: "[{at}{attr1}='{attr1V}']" },
    ...
    },
    {
      ...
      value: "[{at}{attr1}='{attr1V}'][position()={idx}]",
    },
